...
Version: 2.0.1 stabilization release

NovelAgent 2.0.1 stabilizes the 2.0 feature set without expanding the novel-creation surface. It hardens final-artifact integrity, sequential scene evidence, authoritative character/relationship/roster/numeric state, chapter-scoped recovery, context working sets, and byte-for-byte StoryProject writeback while retaining the v1.5-compatible agent loop and opt-in reliable semantic production path.

This release proves the deterministic program-level gates and local regression suite; it is not a claim that unattended long-form production or external model availability is guaranteed. Provider partial responses, latency, and paid first-ten-chapter literary evaluation remain operational concerns outside the local release proof. See the [v2.0.1 stabilization results](docs/v2.0.1-stabilization-results.json) and the [v2.0 baseline and stabilization charter](docs/v2.0-baseline.md).
...
Token admission now binds the configured OpenAI model and endpoint instead of `runtime-default`. Recognized models on the official endpoint use a hash-verified, already-local `tiktoken` asset when available and report `model_tokenizer`, never `provider_exact`; cache misses fall back immediately without downloading. Compatible endpoints, unknown models, and providers without a trusted local tokenizer use a split fallback: the ASCII portion retains a one-token-per-byte floor, while non-ASCII UTF-8 bytes use the `7/18` calibration with the declared 15% margin, followed by 64 framing tokens. The floor therefore still protects JSON syntax, escapes, punctuation, and high-entropy ASCII, but no longer charges every Chinese UTF-8 byte as a token. This is locally tested behavior, not real-provider accuracy evidence.
