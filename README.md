...
Reliability/autonomy claims are tracked by four separate levels—code exists, main-path integration, default enablement, and real verification—in the [Reliability and autonomy capability status](docs/reliability-autonomy-capability-status.md). Synthetic evidence is listed separately and is never treated as real-provider evidence. A complete 50-chapter deterministic simulation passed against code commit `1a2af7d` in 1374.619 seconds; the [retained evidence report](docs/reliability-autonomy-50-chapter-evidence.json) closes only the synthetic long-run gate and does not enable autonomy by default. The billable 1/4/10/20+ gate is documented in [Real autonomy E2E release gates](docs/real-autonomy-e2e.md). One authorized one-chapter attempt on 2026-07-15 failed before chapter completion and produced no successful release report; the four-chapter gate was not started. Workspace `.env` loading is disabled by that harness, and every later billable gate needs a new exact count-bound authorization. No Notion call was made in this upgrade run.

Project movement is also not a default capability. The explicit `--remap-roots` CLI can update an existing EA-global/StoryProject logical data-root binding, with UUID plus revision/digest CAS, only when no transaction is pending and no session is active. It changes the registry only: it does not move files or relocate the StoryProject-embedded Persistence v2 runtime control plane, so whole-project relocation remains unsupported. `RootRegistry` is the unique mutable Event Authority physical-root mapping; immutable historical manifests may retain absolute-path snapshots.
