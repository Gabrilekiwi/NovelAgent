...
| Token counting and calibration | Yes | Yes for model/endpoint binding in the context compiler and durable pre-call reservation. Recognized official OpenAI models use a hash-verified cached `tiktoken` asset as `model_tokenizer`, never `provider_exact`; no asset is downloaded during admission. Compatible/unknown and cache-miss paths use the split fallback | The fallback reserves the ASCII portion with a minimum of `1 token/byte`, the non-ASCII portion at `ceil(7/18 fitted tokens/UTF-8 byte × 1.15)`, then adds 64 framing tokens. There is no one-token floor on non-ASCII UTF-8 bytes | No | Long Chinese production-shaped regressions prove that the fitted value controls its non-ASCII admission rather than degrading to roughly three tokens per character; punctuation, escapes, and high-entropy ASCII retain fail-closed coverage. The fit still comes from `synthetic_acceptance_v1`, is not a mathematical upper bound or real-provider claim, and records `real_provider_verified=false`. Successful Provider usage settles the input reservation; missing output usage preserves at least the full output reservation or visible-text byte fallback, whichever is larger. |
...
| 50-chapter deterministic autonomy simulation | Yes, opt-in test harness | Test-only; it is not a production default | No | Not applicable (synthetic gate) | **Passed (synthetic only).** A clean frozen-source run against code commit `bfa3d04` completed in 1298.864 seconds with 50 outlines/prose publications, chapter Event batches, EA completion entries, publication receipts, Delivery intents/jobs/attempt receipts and required external JSON deliveries, plus 49 linear next-target Arc adjustments. The same clean commit passed 1317 unit tests (7 platform skips) and v1 smoke with 21/21 preflight checks before the simulation. See the [retained evidence report](reliability-autonomy-50-chapter-evidence.json). |
...
`bfa3d04`, closing the synthetic long-run gate only. Autonomy remains opt-in
