...
- **Token admission correction:** the production context compiler and durable model-call reservation now bind the configured provider/model/endpoint. Official recognized OpenAI models use a hash-verified cached `model_tokenizer` when locally available, with no admission-time download; compatible/unknown and cache-miss paths use a split fallback. All ASCII bytes retain the prior one-token floor, while non-ASCII bytes use the calibrated estimate plus its declared safety margin and framing allowance, so JSON/high-entropy ASCII stays protected and natural Chinese is no longer reserved at roughly three tokens per character. Provider usage still replaces the input estimate after a successful response; if output usage is missing, settlement preserves at least the full output reservation or the visible-text UTF-8-byte fallback, whichever is larger. This has local regression evidence only and is not a real-provider accuracy claim.
...
- **Synthetic verification:** a complete 50-chapter deterministic autonomy simulation passed against code commit `bfa3d04` in 1298.864 seconds, with 50 outlines, prose publications, chapter Event batches, EA completion entries, publication receipts, Delivery intents/jobs/attempt receipts, and required external JSON deliveries, plus 49 linear next-target Arc adjustments. See the [retained evidence report](reliability-autonomy-50-chapter-evidence.json). The same clean commit passed 1317 unit tests (7 platform skips) and v1 smoke with 21/21 preflight checks before the simulation. This closes only the synthetic long-run gate.
...
- **Legacy-book migration:** paused by the operator on 2026-07-16, removed from the current goal/current execution backlog/completion conditions, and recorded as deferred work. Existing implementation, schemas, tests, documentation, and read-only preview history are retained. No six-decision review, `MigrationApproval`, migration execution, or activation is pending; the active legacy book remains unmigrated. A later resumption requires an explicit operator decision and a fresh read-only preview.
- **Default enablement:** autonomy and whole-project remap remain explicit/conditional; legacy-book event-authority migration is paused. Neither default autonomous generation, default migration, nor default project portability is claimed.
