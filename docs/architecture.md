...
External publication uses durable delivery jobs, leases, attempt receipts, and reconciliation. Provider retry policy is operation-specific: model reads and Notion queries may retry within bounded deadlines, partial streamed model output never retries, and Notion create remains a single generic attempt. When a model stream fails after yielding text, the runtime stores that fragment with a terminal `partial_response_failed` receipt; restart therefore does not resend the call or treat it as uncertain. A failure before any response content remains `provider_call_uncertain` and blocks new provider work until `--recover-locked-chapter` writes an append-only resolution checkpoint. That recovery command does not forge a provider receipt: it preserves hash-verified complete/partial output or records a reset, and the executor ignores only the explicitly resolved execution ids.
...
`core.engine.repair_convergence` separates repair progress from mere workflow completion. It builds stable validation checkpoints from problem counts, severities, codes/areas, and chapter digests. A transition is converging only when the problem count strictly decreases without a new critical or more-severe finding. Converging transitions issue a content-derived authorization to `RunBudgetTracker`; the tracker remains fail-closed until a repair/validation reservation actually crosses the normal token ceiling, then adds only the exact deficit. Authorizations are stage- and cycle-bounded, never enlarge provider-call/time/cost limits, are included in execution budget evidence, and can be reconstructed from durable non-rejected model-call attempts during restart hydration. Stalled or regressing transitions raise `chapter_repair_not_converging` and restore the best validated draft before the failed run artifact is written.
