...
The current reliability/autonomy claim boundary is the four-level [Reliability and autonomy capability status](reliability-autonomy-capability-status.md). Synthetic checks are not real verification: a complete 50-chapter deterministic simulation passed against code commit `42d4804` in 1306 seconds ([retained report](reliability-autonomy-50-chapter-evidence.json)), and the same clean commit passed 1317 unit tests (7 platform skips) plus v1 smoke with 21/21 preflight checks. The 2026-07-15 single-chapter attempt failed before chapter completion, and no successful 1/4/10/20+ report exists. On 2026-07-16 those future billable runs were moved outside the current Codex goal: the operator will run them manually after goal completion and Codex will only analyze the redacted reports. No Notion call was made in this upgrade run, and the synthetic pass does not enable autonomy by default.

Legacy-book event-authority migration was also paused by the operator on 2026-07-16 and is not current goal work. All existing migration code, schemas, tests, documentation, and read-only preview history remain in place. The active legacy book is not approved, migrated, or activated; resumption requires an explicit later decision and a fresh read-only preview.

Whole-project remap now exists as a conditional post-rename path, not a default portability claim. After an operator-controlled same-volume rename, `--remap-roots` verifies stable StoryProject/logical-root identities, pre-armed locks, UUID plus revision/digest CAS, an absent old path, and an idle project before forward-rebinding every recognized embedded registry with the main registry last. The command moves no data and refuses cross-volume/copy-delete/case-only moves, changed roots, unknown registries, and external mutable EA transaction roots. It has local fault/concurrency evidence only, not a retained real target-project report; immutable historical manifests may retain absolute-path snapshots.
