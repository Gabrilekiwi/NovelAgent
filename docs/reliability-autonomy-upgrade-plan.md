...
本计划建立时的字符截断、bigram 阈值和全局字节 Token 估算均属于待替换方案。2026-07-20 的后续修复已将生产 Token 准入绑定到实际配置的 model/endpoint：官方已知 OpenAI 模型在存在哈希校验的本地资产时使用 `model_tokenizer`，准入路径不会隐式下载；其余路径按 ASCII 与非 ASCII 分区估算。ASCII 仍保留 `1 token/byte` 防护，非 ASCII 使用带安全余量的校准值，因此中文不再被全局字节下限覆盖。该修复有本地回归证据，但仍不视为真实 Provider 准确率验证。
